<x-app-layout>
    <div class="px-4 sm:px-6 lg:px-8 py-8 w-full max-w-9xl mx-auto">
        ...
        <div class="px-8 py-8 bg-white rounded-lg shadow-lg">
            <form action="{{ route('admin.kriteria.update', Crypt::encrypt($kriteria->id)) }}" method="POST">
                @method('PUT')
                @csrf
                ...
                <div class="mb-4">
                    <input type="text" autofocus class="focus:shadow-soft-primary-outline text-sm leading-5.6 ease-soft block w-full appearance-none rounded-lg border border-solid border-gray-300 bg-white bg-clip-padding px-3 py-2 font-normal text-gray-700 transition-all focus:border-blue-400 focus:outline-none focus:transition-shadow" placeholder="Kode Kriteria" name="kode_kriteria" value="{{ $kriteria->kode_kriteria }}" required />
                ...
                <div class="mb-4">
                    <input type="number" autofocus class="focus:shadow-soft-primary-outline text-sm leading-5.6 ease-soft block w-full appearance-none rounded-lg border border-solid border-gray-300 bg-white bg-clip-padding px-3 py-2 font-normal text-gray-700 transition-all focus:border-blue-400 focus:outline-none focus:transition-shadow" placeholder="Bobot" name="bobot" value="{{ $kriteria->bobot }}" required />
                    <x-input-error :messages="$errors->get('bobot')" class="mt-2" />
                </div>
                <div class="flex flex-wrap justify-between">
                    <a href="{{ route('admin.kriteria.index') }}" class="inline-block px-4 py-2 bg-slate-500 rounded-lg text-white shadow-sm hover:opacity-80 ease-in-out transition duration-300"><i class="fa-solid fa-chevron-left"></i> Kembali</a>
